...
Learn how to use Model Context Protocol (MCP) servers with ZL Universe API.

> [!NOTE]
> Requires ZL Universe 0.4.0 or newer.

ZL Universe supports Model Context Protocol (MCP) usage via API. MCP allows models to interact with external tools and services through standardized servers.
...
> [!INFO]
> Ephemeral MCP servers require the "Allow per-request MCPs" setting to be enabled in Server Settings.
...
![MCP editor interface](placeholder)
