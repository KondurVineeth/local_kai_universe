import { Brain, Compass, Cube, Eye, FunnelSimple, SealCheck, Wrench } from '@phosphor-icons/react';
...
import { selectDiscoverCategory } from '../../store/selectors';
import { categorySet, type DiscoverCategory } from '../../store/slice';

interface NavItem {
  readonly key: DiscoverCategory;
...
const PRIMARY: readonly NavItem[] = [
  { key: 'staff-picks', label: 'Staff Picks', icon: SealCheck, hint: 'Curated' },
  { key: 'all', label: 'View All', icon: Compass },
];

const FILTERS: readonly NavItem[] = [
...
export function DiscoverSecondarySidebar() {
  const active = useAppSelector(selectDiscoverCategory);
  const dispatch = useAppDispatch();
  return (
    <SecondarySidebar title="Discover">
...
function NavRow({
  ...
  return (
    ...
    >
      ...
      {item.hint && <span className="text-[10px] text-fg-subtle">{item.hint}</span>}
    </button>
  );
}
